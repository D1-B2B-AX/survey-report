...
- **강의관리시트/싱크업 문서에서 읽는 탭은 "강의요약"과 "교육 개요" 2종류만입니다.** 나머지 탭(운영 표준, 체크리스트, 운영 캘린더 등)은 읽지 않습니다. "교육 개요" 탭에서는 키워드가 포함된 행(라벨 셀)을 찾아 인접 셀의 값을 읽습니다: 과정명, 교육형태, 교육일정, 교육시수, 교육대상, 강사명("강사님"/"강사"/"강사명" 키워드 포함 행, 상단+하단 담당자 목록 모두 탐색).
...
- **운영진 의견만 LLM 초안 생성이 허용됩니다.** 근거 없는 주장은 금지, 반드시 출처를 명시합니다. 객관식/주관식에서 이미 보여준 내용을 반복하지 않습니다. (구체적 수치 인용 포함 — "어렵게 느낀 수강생이 많았다" 같은 정성적 맥락 언급은 허용)
...
│   └── generate-shell.js  ← HTML 셸 생성 + 조립 (모드1: head/tail/헤더 상수 반환, 모드2: head+헤더(meta)+본문+hidden(meta)+tail 조립)
└── templates/
...
- **GAS 연동용 hidden 필드**: generate-shell.js가 meta.json에서 자동 삽입 (hidden-meta, hidden-start-time, hidden-html-created-time, hidden-phase5, hidden-plugin-version, hidden-estimated-manual-time, hidden-mail-copied). OM에게 미표시. GAS 전송 순서: startTime → htmlCreatedTime → copyClickTime → company → course → date → instructor → respondents → omName → mailCopied → modified → modifiedAreas → phase5Log → pluginVersion → estimatedManualTime
...
# HTML 조립 (meta.json 포함 — 헤더+hidden 자동 삽입)
cd ~/.claude/skills/report && node scripts/generate-shell.js "제목" "본문파일경로" "출력파일경로" "meta.json경로"
```
